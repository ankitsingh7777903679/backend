...
exports.reportService = {
    getFinancialSummary: async (instituteId, monthFilter) => {
        const [fees, classes, leadsCount, students, exams, examResults, attendanceLogs] = await Promise.all([
            ...
            exam_model_1.Exam.find({ instituteId, status: { $ne: "deleted" } }),
            examResult_model_1.ExamResult.find({ instituteId, status: { $ne: "deleted" } }),
            attendance_model_1.Attendance.find({ instituteId }),
        ]);
        ...
        exams.forEach((e) => {
            ...
            if (resultsForExam.length > 0) {
                const examAvgPct = resultsForExam.reduce((sum, r) => sum + (r.totalMarks > 0 ? (r.marksObtained / r.totalMarks) * 100 : 0), 0) /
    ...
    getStudentProgressReport: async (instituteId, studentId, monthFilter) => {
        const [student, institute] = await Promise.all([
            student_model_1.Student.findOne({ _id: studentId, instituteId, status: { $ne: "deleted" } }),
            institute_model_1.Institute.findById(instituteId),
        ]);
        if (!student) {
        ...
        const homeworkList = await homework_model_1.Homework.find({
            instituteId,
            status: "active",
            $or: [
        ...
        const submissions = await homeworkSubmission_model_1.HomeworkSubmission.find({
        ...
        const examResults = await examResult_model_1.ExamResult.find({
            studentId: student._id,
            instituteId,
            status: "active",
        }).sort({ createdAt: -1 });
        const examIds = examResults.map((r) => r.examId);
        const exams = await exam_model_1.Exam.find({ _id: { $in: examIds } });
        ...
        const allBatchStudents = await student_model_1.Student.find({
            instituteId,
            status: "active",
